...
<body>
<div id="path">
    Административен панел >> Страници >> Срокове
</div>
<div class="main-area">

    <c:url var="saveUrl" value="/cms/admin/footer/limits" />
    <form:form method="post" modelAttribute="limits" action="${saveUrl}">


        <h2>Българска версия</h2>
        <textarea class="ckeditor" cols="80" id="editorBG" name="limitsBG" rows="10" ><c:if test="${not empty limits}"> ${limits.limits.map['bg'].text}</c:if></textarea>
        <h2>English version</h2>
        <textarea class="ckeditor" cols="80" id="editorEN" name="limitsEN" rows="10" ><c:if test="${not empty limits}"> ${limits.limits.map['en'].text}</c:if></textarea>
