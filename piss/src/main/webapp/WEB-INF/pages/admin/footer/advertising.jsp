...
<body>
<div id="path">
    Административен панел >> Страници >> Реклама
</div>
<div class="main-area">

    <c:url var="saveUrl" value="/cms/admin/footer/advertising" />
    <form:form method="post" modelAttribute="advertising" action="${saveUrl}">


        <h2>Българска версия</h2>
        <textarea class="ckeditor" cols="80" id="editorBG" name="advertisingBG" rows="10" ><c:if test="${not empty advertising}"> ${advertising.adds.map['bg'].text}</c:if></textarea>
        <h2>English version</h2>
        <textarea class="ckeditor" cols="80" id="editorEN" name="advertisingEN" rows="10" ><c:if test="${not empty advertising}"> ${advertising.adds.map['en'].text}</c:if></textarea>
