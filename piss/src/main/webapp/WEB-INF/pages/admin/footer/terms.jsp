...
<body>
<div id="path">
    Административен панел >> Страници >> Условия за ползване
</div>
<div class="main-area">

    <c:url var="saveUrl" value="/cms/admin/footer/terms" />
    <form:form method="post" modelAttribute="terms" action="${saveUrl}">


        <h2>Българска версия</h2>
        <textarea class="ckeditor" cols="80" id="editorBG" name="termsBG" rows="10" ><c:if test="${not empty terms}"> ${terms.termsOfUse.map['bg'].text}</c:if></textarea>
        <h2>English version</h2>
        <textarea class="ckeditor" cols="80" id="editorEN" name="termsEN" rows="10" ><c:if test="${not empty terms}"> ${terms.termsOfUse.map['en'].text}</c:if></textarea>
