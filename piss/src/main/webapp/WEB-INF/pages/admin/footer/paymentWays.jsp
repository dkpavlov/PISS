...
<body>
<div id="path">
    Административен панел >> Страници >> Начини на плащане
</div>
<div class="main-area">

    <c:url var="saveUrl" value="/cms/admin/footer/paymentWays" />
    <form:form method="post" modelAttribute="paymentWays" action="${saveUrl}">


        <h2>Българска версия</h2>
        <textarea class="ckeditor" cols="80" id="editorBG" name="paymentWaysBG" rows="10" ><c:if test="${not empty paymentWays}"> ${paymentWays.waysOfPayment.map['bg'].text}</c:if></textarea>
        <h2>English version</h2>
        <textarea class="ckeditor" cols="80" id="editorEN" name="paymentWaysEN" rows="10" ><c:if test="${not empty paymentWays}"> ${paymentWays.waysOfPayment.map['en'].text}</c:if></textarea>
